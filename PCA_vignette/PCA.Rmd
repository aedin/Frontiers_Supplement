...
We will demonstrate some of these and explore these using the `explor` package.
...
`ade4::dudi.pca` and `prcomp` by default both perform correlation-based PCA as they center and scale the data prior to SVD, with `center = TRUE`, `scale = TRUE`. In contrast, `princomp` by default performs covariance-based PCA, only centering the data without scaling, as it sets `cor=FALSE` by default. 


#  Toy data
Create a cloud of points; two vectors, `x` and `y` of length 100. 
...
PCA can be computed as a singular value decomposition of a column centered matrix. Therefore we first process the matrix.  In this example, we don't scale. This is not advised.
...
First `stats::prcomp`.  The eigenvectors are stored in \$rotation. Note these are the same as svd$v on scaled data 
```{r}
...
If we had more components, we could generate a scree plot. It's not very useful with 2 components, but here is the code
...
# princomp
`princomp` was written for compatiblity with S-PLUS however it is not recommended. Its is better to use prcomp or svd. That is because by default princomp performs a decomposition of the covariance not correlation matrix. Princomp can call eigen on the correlation or covariance matrix. Its  default calculation uses divisor N for the covariance matrix.
...
```
The summary printout is equivalent to P3 (p3\$eig) above. The eigenvalues are stored in p4\$eig.
...
coefficients of the combination or the loadings of variables.  Equally the output matrix l1 gives the individuals’ coordinates, normed to 1. It is also called the
